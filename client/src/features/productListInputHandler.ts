...
import { MOCK_INGREDIENTS } from '../variables';
import { deleteProduct, filterOptions, moveToStockedHandler } from './productHandlerHelpers';
...
function createListItem(product: string): HTMLElement {
  ...
  listItem.querySelector('.product-controls__delete-btn')?.addEventListener('click', e => {
    ...
    if (MOCK_INGREDIENTS.includes(productName)) {
